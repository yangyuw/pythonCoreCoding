...
- **新式类**: 在2.2以前类和类型是不同的,为了统一类和类型,引入新式类,更多的内置属性将会被引入, 描述符的引入, 属性可以用来计算等等. 用户默认定义的是经典类, 新式类需要继承自所有的类的基类(object)或者继承自object的新类.    在旧式类中，所有的 类对象的类型都被视为 classobj 类型， 所有的实例对象的类型都被视为 instance 类型；而所有实例对象的类型都是对应的类对象，所有实例对象的类都是对应的类对象,也就是说python中新式类的所有对象(包括类对象，实例对象)所拥有**的类和类型都统一了**。
